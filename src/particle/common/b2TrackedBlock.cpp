...
#include "particle/common/b2TrackedBlock.h"
#include "core.h"
#include <stddef.h>
#include <stdint.h>
#include <new>
...
void* b2TrackedBlock::Allocate(uint32 size)
{
	...
	b2TrackedBlock* block = (new(memory) b2TrackedBlock);
	block->m_size = sizeof(b2TrackedBlock) + size;
...
{
	b2Assert(block);
	const std::size_t size = block->m_size;
